...
SELECT
	...
	--IF OUTGOING HAS AN ENTRY
	,(CASE WHEN (Select SUM(VPM2.SumApplied) From VPM2 where VPM2.DocEntry = T1.DocEntry AND T1.TaxDate BETWEEN @1stMosStr AND @1stMosStp) <>
				(Select SUM(OPCH.DocTotal) From OPCH where OPCH.DocEntry = T1.DocEntry AND T1.TaxDate BETWEEN @1stMosStr AND @1stMosStp) THEN

		(Cast((Select SUM(VPM2.SumApplied) / SUM(OPCH.DocTotal) * SUM(OPCH.BaseAmnt)
		from OPCH
		JOIN VPM2 on OPCH.DocEntry = VPM2.DocEntry 
		Where OPCH.DocNum = T1.DocNum 
		AND T1.TaxDate BETWEEN @1stMosStr AND @1stMosStp)as DECIMAL(16,2)))		
	...
	--IF OUTGOING HAS AN ENTRY
,		(CASE WHEN (Select SUM(VPM2.U_WTAXPay) From VPM2 where VPM2.DocEntry = T1.DocEntry AND T1.DocDate BETWEEN @1stMosStr AND @1stMosStp) > 0 THEN
		ISNULL(CAST((Select SUM(VPM2.U_WTAXPay) From VPM2 where VPM2.DocEntry = T1.DocEntry)AS DECIMAL(16,2)),0)
	...
	--IF OUTGOING HAS AN ENTRY
	,(CASE WHEN (Select SUM(VPM2.SumApplied) From VPM2 where VPM2.DocEntry = T1.DocEntry AND T1.TaxDate BETWEEN @2ndMosStr AND @2ndMosStp) <>
				(Select SUM(OPCH.DocTotal) From OPCH where OPCH.DocEntry = T1.DocEntry AND T1.TaxDate BETWEEN @2ndMosStr AND @2ndMosStp) THEN

		(Cast((Select SUM(VPM2.SumApplied) / SUM(OPCH.DocTotal) * SUM(OPCH.BaseAmnt)
		from OPCH
		JOIN VPM2 on OPCH.DocEntry = VPM2.DocEntry 
		Where OPCH.DocNum = T1.DocNum 
		AND T1.TaxDate BETWEEN @2ndMosStr AND @2ndMosStp)as DECIMAL(16,2)))		
	...
	--IF OUTGOING HAS AN ENTRY
,		(CASE WHEN (Select SUM(VPM2.U_WTAXPay) From VPM2 where VPM2.DocEntry = T1.DocEntry AND T1.DocDate BETWEEN @2ndMosStr AND @2ndMosStp) > 0 THEN
		ISNULL(CAST((Select SUM(VPM2.U_WTAXPay) From VPM2 where VPM2.DocEntry = T1.DocEntry)AS DECIMAL(16,2)),0)
	...
	--IF OUTGOING HAS AN ENTRY
	,(CASE WHEN (Select SUM(VPM2.SumApplied) From VPM2 where VPM2.DocEntry = T1.DocEntry AND T1.TaxDate BETWEEN @3rdMosStr AND @3rdMosStp) <>
				(Select SUM(OPCH.DocTotal) From OPCH where OPCH.DocEntry = T1.DocEntry AND T1.TaxDate BETWEEN @3rdMosStr AND @3rdMosStp) THEN

		(Cast((Select SUM(VPM2.SumApplied) / SUM(OPCH.DocTotal) * SUM(OPCH.BaseAmnt)
		from OPCH
		JOIN VPM2 on OPCH.DocEntry = VPM2.DocEntry 
		Where OPCH.DocNum = T1.DocNum 
		AND T1.TaxDate BETWEEN @3rdMosStr AND @3rdMosStp)as DECIMAL(16,2)))
	...
	--third month of the quarter
	--IF OUTGOING HAS AN ENTRY
,		(CASE WHEN (Select SUM(VPM2.U_WTAXPay) From VPM2 where VPM2.DocEntry = T1.DocEntry AND T1.DocDate BETWEEN @3rdMosStr AND @3rdMosStp) > 0 THEN
		ISNULL(CAST((Select SUM(VPM2.U_WTAXPay) From VPM2 where VPM2.DocEntry = T1.DocEntry)AS DECIMAL(16,2)),0)
