    ...
    ,(CASE WHEN (SELECT OCRD.U_taxPayerClass FROM OCRD WHERE OCRD.CardCode = T2.CardCode AND OCRD.ValidComm is null) <> 'I' THEN
	 ...
	 ,T1.U_ATCName AS [NoP]
	  ...
	  --first Month of the Quarter
		  ,ISNULL(CAST((SELECT RCT2.SumApplied 
		  FROM RCT2 
		  WHERE RCT2.DocEntry = T0.DocEntry AND T2.TaxDate BETWEEN @1stMosStr AND @1stMosStp)AS DECIMAL(16,2)),0) AS [1stMosIP]
		  ...
		  WHERE JDT1.U_DOCNUM = T1.U_DOCNUM AND T2.TaxDate BETWEEN @1stMosStr AND @1stMosStp)AS DECIMAL(16,2)),0) AS [1stMosTR]
	  	  ...
	  	  ,ISNULL(CAST((SELECT RCT2.U_WtaxPay 
		  FROM RCT2 
		  WHERE RCT2.DocEntry = T0.DocEntry AND T2.TaxDate BETWEEN @1stMosStr AND @1stMosStp)AS DECIMAL(16,2)),0) AS [1stMosTW]
	  --end first Month of the Quarter

	  --second Month of the Quarter
	  ,ISNULL(CAST((SELECT RCT2.SumApplied 
		  FROM RCT2 
		  WHERE RCT2.DocEntry = T0.DocEntry AND T2.TaxDate BETWEEN @2ndMosStr AND @2ndMosStp)AS DECIMAL(16,2)),0) AS [2ndMosIP]
	  ...
	  ,ISNULL(CAST((SELECT RCT2.U_WtaxPay 
		  FROM RCT2 
		  WHERE RCT2.DocEntry = T0.DocEntry AND T2.TaxDate BETWEEN @2ndMosStr AND @2ndMosStp)AS DECIMAL(16,2)),0) AS [2ndMosTW]
	 --end second Month of the Quarter

	  --third Month of the Quarter
	  ,ISNULL(CAST((SELECT RCT2.SumApplied 
		  FROM RCT2 
		  WHERE RCT2.DocEntry = T0.DocEntry AND T2.TaxDate BETWEEN @3rdMosStr AND @3rdMosStp)AS DECIMAL(16,2)),0) AS [3rdMosIP]
	  ...
	  ,ISNULL(CAST((SELECT RCT2.U_WtaxPay 
		  FROM RCT2 
		  WHERE RCT2.DocEntry = T0.DocEntry AND T2.TaxDate BETWEEN @3rdMosStr AND @3rdMosStp)AS DECIMAL(16,2)),0) AS [3rdMosTW]
