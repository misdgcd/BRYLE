...
--	,ISNULL((T1.U_CUSTOMER),T1.U_ALIAS_VENDOR) AS [ALIAS]
	...
	,ISNULL(CAST((SELECT INV5.TaxbleAmnt
		...
		AND T1.DocDate BETWEEN @1stMosStr AND @1stMosStp) AS DECIMAL(16,2)) ,0)AS [1stMosIP]
	...
	,ISNULL(CAST((SELECT INV5.TaxbleAmnt
		...
		WHERE INV5.AbsEntry = T1.DocNum
		AND INV5.WTCode = T0.WTCode
		AND T1.DocDate BETWEEN @2ndMosStr AND @2ndMosStp) AS DECIMAL(16,2)) ,0)AS [2ndMosIP]
	,ISNULL(CAST((SELECT INV5.Rate
		FROM INV5
		WHERE INV5.AbsEntry = T1.DocNum
		AND INV5.WTCode = T0.WTCode
	...
	,ISNULL(CAST((SELECT INV5.TaxbleAmnt
		...
		WHERE INV5.AbsEntry = T1.DocNum
		AND INV5.WTCode = T0.WTCode
		AND T1.DocDate BETWEEN @3rdMosStr AND @3rdMosStp) AS DECIMAL(16,2)) ,0)AS [3rdMosIP]
	,ISNULL(CAST((SELECT INV5.Rate
		FROM INV5
		WHERE INV5.AbsEntry = T1.DocNum
		AND INV5.WTCode = T0.WTCode
...
SELECT
	...
	,ISNULL(CAST((SELECT -1 * RIN5.WTAmnt
		...
		WHERE RIN5.AbsEntry = T1.DocNum
		AND RIN5.WTCode = T0.WTCode
		AND T1.DocDate BETWEEN @1stMosStr AND @1stMosStp) AS DECIMAL(16,2)) ,0) AS [1stMosTW]
	--end first Month of the Quarter
	--second Month of the Quarter
	,ISNULL(CAST((SELECT -1 *RIN5.TaxbleAmnt
		FROM RIN5
	...
	,ISNULL(CAST((SELECT -1 * RIN5.WTAmnt
		...
		WHERE RIN5.AbsEntry = T1.DocNum
		AND RIN5.WTCode = T0.WTCode
		AND T1.DocDate BETWEEN @2ndMosStr AND @2ndMosStp) AS DECIMAL(16,2)) ,0) AS [2ndMosTW]
	--end second Month of the Quarter
	--third Month of the Quarter
	,ISNULL(CAST((SELECT -1 * RIN5.TaxbleAmnt
		FROM RIN5
	...
	,ISNULL(CAST((SELECT -1 * RIN5.WTAmnt
		...
		WHERE RIN5.AbsEntry = T1.DocNum
		AND RIN5.WTCode = T0.WTCode
		AND T1.DocDate BETWEEN @3rdMosStr AND @3rdMosStp) AS DECIMAL(16,2)) ,0) AS [3rdMosTW]
	--end third Month of the Quarter
	----total
